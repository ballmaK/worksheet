        ...
        <el-table :data="filteredTeams" style="width: 100%" v-loading="loading">
          ...
          <el-table-column label="操作" width="300" fixed="right">
            <template #default="{ row }">
              ...
              <el-button size="small" type="primary" @click="manageTeam(row)" v-if="canManageTeam(row)">
                管理
              </el-button>
              <el-button size="small" type="warning" @click="editTeam(row)" v-if="canEditTeam(row)">
                编辑
              </el-button>
              <el-button size="small" type="danger" @click="deleteTeam(row)" v-if="canDeleteTeam(row)">
                删除
              </el-button>
            </template>
          </el-table-column>
                ...
                <el-dropdown @command="handleTeamAction">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      ...
                      <el-dropdown-item :command="{ action: 'manage', team }" v-if="canManageTeam(team)">
                        管理
                      </el-dropdown-item>
                      <el-dropdown-item :command="{ action: 'edit', team }" v-if="canEditTeam(team)">
                        编辑
                      </el-dropdown-item>
                      <el-dropdown-item :command="{ action: 'delete', team }" v-if="canDeleteTeam(team)" divided>
                        删除
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="handleCreateTeamSubmit" :loading="creating">
            创建
          </el-button>
        </span>
      </template>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="handleEditTeamSubmit" :loading="editing">
            确定
          </el-button>
        </span>
      </template>
...
const createRules: FormRules = {
  ...
  description: [
    { max: 200, message: '团队描述长度不能超过 200 个字符', trigger: 'blur' }
  ]
}
    ...
    const teamsWithDetails = await Promise.all(
      teamsData.map(async (team) => {
        const [members, projects] = await Promise.all([
          ...
          projectApi.getProjects(team.id)
        ])
        ...
        if (currentMember) {
          my_role = currentMember.role
          is_member = true
          is_admin = currentMember.role === 'team_admin'
          is_my_team = team.creator_id === currentUserId
        }
        
        // 添加模拟数据（实际应该从API获取）
        const active_projects = Math.floor(Math.random() * 5) + 1
        const pending_invites = Math.floor(Math.random() * 3)
        const recent_activities = Math.floor(Math.random() * 20) + 5
...
const handleCreateTeamSubmit = async () => {
  ...
  try {
    creating.value = true
    console.log('提交数据:', createForm.value)
    await teamApi.createTeam(createForm.value)
    ...
    ElMessage.error('创建团队失败: ' + (error.response?.data?.detail || error.message))
  } finally {
...
const handleEditTeamSubmit = async () => {
  ...
  try {
    editing.value = true
    await teamApi.updateTeam(editForm.value.id, {
    ...
    ElMessage.error('更新团队失败: ' + (error.response?.data?.detail || error.message))
  } finally {
    editing.value = false
  }
}
...
const deleteTeam = async (team: any) => {
  try {
    ...
    if (error !== 'cancel') {
      ElMessage.error('删除团队失败: ' + error.message)
    }
  }
}
...
const canManageTeam = (team: any) => {
  return team.is_admin || team.is_my_team
}
